...
static int controller_read_buttons(uint8_t *pressure)
{
    ...
    if ((g_controller_data.buttons & 0x3F) != 0)
        g_brg_current = g_brg_active;
    else
        g_brg_current = g_brg_inactive;
...
/* chusan in x86 has to open the device and create the shmem */
    ...
    if (hid_open_device(&g_device_handle, VID, PID, 0) != 0 && hid_open_device(&g_device_handle, VID, PID, -1) != 0) /* hori official controller has only one interface */
    {
